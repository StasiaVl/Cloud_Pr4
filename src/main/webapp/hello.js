...
function enableButtons () {
	...
	btn = document.getElementById("input_greet_by_name_surname");
	btn.onclick = function(){greetByNameSurname();};

	btn.value="Click me for greeting by name and surname";
}
...
function greetByNameSurname () {
    var name = document.getElementById("name_field").value;
    var surname = document.getElementById("surname_field").value;

    var request = gapi.client.helloworldendpoints.greetByNameSurname({'name':name, 'surname':surname, 'times':1});
    request.execute(sayHelloCallback);
}
